...
class MockIAnswerOptionWatcherApi extends Mock implements IAnswerOptionWatcherApi {}

void main() {
  late AnswerOptionWatcherCubit cubit;
  late MockIAnswerOptionWatcherApi repo;

  setUp(() {
    repo = MockIAnswerOptionWatcherApi();
    cubit = AnswerOptionWatcherCubit(repo);
  });
  ...
  group('watchOneStarted()', () {
    blocTest(
      ...
      ''',
      build: () {
        when(() => repo.watchOne(answerOptionDayGood.id)).thenAnswer(
            (_) => Stream.value(const Left(CrudFailure.unexpected())));
        return cubit;
      },
      act: (_) => cubit.watchOneStarted(answerOptionDayGood.id),
      expect: () => [
        const EntityWatcherState.loadInProgress(),
        const EntityWatcherState.loadFailure(CrudFailure.unexpected()),
      ],
      verify: (_) => repo.watchOne(answerOptionDayGood.id),
    );
    blocTest(
      '''should call repo.watchOne() and 
      emit:
    ...
    blocTest(
      ...
      build: () {
        when(() => repo.watchOne(answerOptionDayGood.id))
      ...
      expect: () => [
        const EntityWatcherState.loadInProgress(),
        EntityWatcherState<AnswerOption>.loadSuccess(listOf(answerOptionDayGood)),
      ],
      verify: (_) => repo.watchOne(questionDay.id),
    );
  });
  group('watchAllOfChatBotStarted()', () {
    blocTest(
      '''should call repo.watchAllOfChatBot() and 
      emit:
      [LoadInProgress],
      [LoadFailure]
      when repo returns failure
      ''',
      build: () {
        when(() => repo.watchAllOfChatBot(chatBotDiary.id)).thenAnswer(
            (_) => Stream.value(const Left(CrudFailure.unexpected())));
        return cubit;
      },
      ...
      verify: (_) => repo.watchAllOfChatBot(chatBotDiary.id),
    );
    blocTest(
      '''should call repo.watchAllOfChatBot() and 
      emit:
    ...
    blocTest(
      ...
      [LoadSuccess]
       ...
       (repo.watchOne() is successful)''',
      build: () {
        when(() => repo.watchAllOfChatBot(chatBotDiary.id))
            .thenAnswer((_) => Stream.value(Right(listOf(answerOptionDayGood))));
        return cubit;
      },
      act: (_) => cubit.watchAllOfChatBotStarted(chatBotDiary.id),
  ...
  group('watchAllOfQuestionStarted()', () {
    blocTest(
      ...
      ''',
      build: () {
        when(() => repo.watchAllOfParentEntity(questionDay.id)).thenAnswer(
            (_) => Stream.value(const Left(CrudFailure.unexpected())));
        return cubit;
      },
      act: (_) => cubit.watchAllOfQuestionStarted(questionDay.id),
      expect: () => [
        const EntityWatcherState.loadInProgress(),
        const EntityWatcherState.loadFailure(CrudFailure.unexpected()),
      ],
      verify: (_) => repo.watchAllOfParentEntity(questionDay.id),
    );
    blocTest(
      '''should call repo.watchAllOfParentEntity() and 
      emit:
    ...
    blocTest(
      ...
      [LoadSuccess]
       ...
       (repo.watchOne() is successful)''',
      build: () {
        when(() => repo.watchAllOfParentEntity(questionDay.id))
            .thenAnswer((_) => Stream.value(Right(listOf(answerOptionDayGood))));
        return cubit;
      },
      act: (_) => cubit.watchAllOfQuestionStarted(questionDay.id),
